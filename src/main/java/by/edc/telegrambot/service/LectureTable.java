...
public class LectureTable {
  ...
  public static String getLectureTable(String message, TableModel tableModel) throws IOException {
    ...
    URL getLecturesTable = new URL(
        "https://journal.bsuir.by/api/v1/portal/employeeSchedule?employeeId=" + tableModel.getId());
        ...
        for (int i = 0; i < getScheduleArray.length(); i++) {
          ...
          for (int j = 0; j < getWeekNumber.length(); j++) {
            ...
            if ((int) getWeekNumber.get(j) == currentWeek) {
              ...
              result +=
                  "*" + tableModel.getStudentGroup() + "*" + " " + tableModel.getLessonTime() + " "
                      +
    ...
    if (workDay) {
      return "Ищу расписание для преподавателя " + tableModel.getFio()
          + " на завтра, учебная неделя №" + currentWeek + "\n"
          + "\n" + result;
    } else {
